...
def oversampling(X, y):
    (X_resampled, y_resampled) = SMOTE(random_state=42).fit_resample(X,
            y)
...
def getBestParams(task, dataset_name):
    ...
    best_params = baseline1[(baseline1['Name'] == dataset_name)
                            & (baseline1['Task'] == task)]

    if len(best_params) < 1:
        return dict(clf__C=1428.5715142857143, clf__penalty='l2',
                    vect__max_df=0.8, vect__max_features=None,
                    vect__stop_words=None)

    max_features = best_params['max features'].values[0]

    model_params = {
        'vect__max_features': (max_features if max_features != 'None'
                               and not pd.isnull(max_features) else None),
        'vect__max_df': (best_params['max df'
                         ].values[0] if not pd.isnull(best_params['max df'
                         ].values[0]) else 1),
        'clf__C': (best_params['C'
                   ].values[0] if not pd.isnull(best_params['C'
                   ].values[0]) else 1000.0),
        'clf__penalty': (best_params['P'
                         ].values[0] if not pd.isnull(best_params['P'
                         ].values[0]) else 'l2'),
        }
...
def model(
    ...
    vect = TfidfVectorizer(max_features=params.get('vect__max_features'
                           ), max_df=params.get('vect__max_df'))
    ...
    for (train_index, test_index) in K.split(X, y):
        ...
        clf = LogisticRegression(C=params.get('clf__C'),
                                 penalty=params.get('clf__penalty'),
                                 solver='liblinear')
    ...
    report = pd.DataFrame(classification_report(expected_y,
                          predicted_y, digits=5,
                          target_names=classes_name, output_dict=True))
    report = report.transpose()

    return (report, np.asarray(expected_y), np.asarray(predicted_y),
            np.asarray(score_y))
...
def run(
    ...
    (X, _, y, _) = loadTrainTest(task=task, dataset_name=dataset_name,
                                 root=root, lang=lang)
    ...
    (report, expected_y, predicted_y, score_y) = model(X, y, n_classes,
            classes_name, params)
    ...
    plot_confusion_matrix(c_matrix, classes_name, task, dataset_name
                          + '_' + lang, True)
    cm = pd.DataFrame(c_matrix, columns=classes_name,
                      index=classes_name)
...
if __name__ == '__main__':
    ...
    if g_task is not None:
        ...
        processes = [mp.Process(target=run, args=(x[0], x[1], x[2],
                     x[3])) for x in args]
